...
class StaticBlackHoleBase:
    ...
    def __init__(self, scenario_name: str | None, **kwargs):
        ...
        self.victim_ip = None
        routers = self.net_env.routers or []
        for router in random.sample(routers, len(routers)):
            ...
            if connected_hosts:
                ...
                break
        if not self.faulty_devices:
            raise RuntimeError(
                f"{self.root_cause_name}: no router with a connected host found in scenario "
                f"'{scenario_name}' (routers={routers})."
            )
    ...
    def __init__(self, scenario_name: str | None, **kwargs):
        ...
        for router in random.sample(routers, len(routers)):
            ...
            if connected_hosts:
                ...
                self.faulty_devices = []
                self.victim_device = None
        if not self.faulty_devices or not self.victim_ip:
            raise RuntimeError(
                f"{self.root_cause_name}: no router with a connected host (with a usable IP) found "
                f"in scenario '{scenario_name}' (routers={routers})."
            )
